...
static void BSC2_pullup_setting(bool v_setval)
{
	struct pin_config BSC2_SCL2_config, BSC2_SDL2_config;
	BSC2_SCL2_config.name = PN_BSC2_SCL;
	BSC2_SDL2_config.name = PN_BSC2_SCL;
	pinmux_get_pin_config(&BSC2_SCL2_config);
	pinmux_get_pin_config(&BSC2_SDL2_config);

	if(v_setval) // pull-up
	{
	    BSC2_SCL2_config.reg.b_i2c.pup_2_0 = 1;
	    BSC2_SDL2_config.reg.b_i2c.pup_2_0 = 1;
	}
	else // no-pull
	{
	    BSC2_SCL2_config.reg.b_i2c.pup_2_0 = 0;
	    BSC2_SDL2_config.reg.b_i2c.pup_2_0 = 0;
	}

	pinmux_set_pin_config(&BSC2_SCL2_config);
	pinmux_set_pin_config(&BSC2_SDL2_config);
}
	...
	do {
		...
		if (msg[0] > 1 && msg[0] < 12) {
			...
			data->touch_is_pressed_arr[msg[0] - 2] = 0;
				...
				{
					data->touch_is_pressed_arr[id] = TSP_STATE_PRESS;
					g_touch_pressed=1;
				}
				else if (msg[1] & MOVE_MSG_MASK)
				{
					data->touch_is_pressed_arr[id] = TSP_STATE_MOVE;
					g_touch_pressed=1;
				}
...
extern int lcd_ld9040_power(int power);
...
static void mxt224_late_resume(struct early_suspend *h)
{
	...
	mxt224_irq_thread(data->client->irq, data);
...
static int read_all_data(struct mxt224_data *data, uint16_t dbg_mode)
{
	...
	msleep(50);		/* msleep(20);  */
	if (data->family_id == 0x81) {
		max_value = max_value + ADC_TOP_VALUE;
		min_value = min_value + ADC_TOP_VALUE;
	}
	...
	for (read_page = 0; read_page < 4; read_page++) {
		for (node = 0; node < 64; node++) {
			...
			if (data->family_id == 0x81) {
				...
				if ((data->qt_refrence_node[num] > MAX_VALUE)
				    ||
				    (data->qt_refrence_node[num] < MIN_VALUE)) {
					state = 1;
					pr_err(
...
static void mxt224_optical_gain(struct mxt224_data *data, uint16_t dbg_mode)
{
	...
	for (read_page = 0; read_page < 4; read_page++) {
		for (node = 0; node < 64; node++) {
			...
			if (data->family_id == 0x81)
				qt_refrence = qt_refrence - ADC_TOP_VALUE;
...
#ifdef SEC_TSP_FACTORY_TEST
static void set_cmd_result(struct mxt224_data *info, char *buff, int len)
{
...
static void set_default_result(struct mxt224_data *info)
{
	char delim = ':';
...
{
	struct mxt224_data *info = (struct mxt224_data *)device_data;
	char buff[16] = {0};
...
static void get_fw_ver_bin(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%#02x", info->tsp_version_disp);
...

{
	...
	snprintf(buff, sizeof(buff), "%s", info->tsp_config_version);
...
static void get_threshold(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%d", info->threshold);
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s", "MXT224-E");
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void run_reference_read(void *device_data)
{
	...
	/* Page Num Clear */
	diagnostic_chip(info, QT_CTE_MODE);
	msleep(30);	

	diagnostic_chip(info, QT_REFERENCE_MODE);
	msleep(30);	

	ret = get_object_info(info, DEBUG_DIAGNOSTIC_T37, &size, &object_address);
	
	msleep(50);	
	if (info->family_id == 0x81) {
		max_value = max_value + ADC_TOP_VALUE;
		min_value = min_value + ADC_TOP_VALUE;
	}
	...
	for (read_page = 0; read_page < 4; read_page++) {
		for (node = 0; node < 64; node++) {
			read_point = (node * 2) + 2;
			read_mem(info, object_address + (u16) read_point, 2, data_buffer);
			info->qt_refrence_node[num] = ((uint16_t) data_buffer[1] << 8) + (uint16_t) data_buffer[0];
			
			if (info->family_id == 0x81) {
				if ((info->qt_refrence_node[num] > MAX_VALUE +	ADC_TOP_VALUE) || (info->qt_refrence_node[num] < MIN_VALUE + ADC_TOP_VALUE)) {
					pr_err("[TSP] Mxt224-E data->qt_refrence_node[%3d] = %5d\n",num, info->qt_refrence_node[num] - ADC_TOP_VALUE);
				}
			} else {
				if ((info->qt_refrence_node[num] > MAX_VALUE) || (info->qt_refrence_node[num] < MIN_VALUE)) {
					pr_err("[TSP] Mxt224 data->qt_refrence_node[%3d] = %5d\n",num, info->qt_refrence_node[num] - ADC_TOP_VALUE);
				}
}

			gv_refrence_node[num] = info->qt_refrence_node[num] - ADC_TOP_VALUE;


	//		printk("[TSP] gv_refrence_node[%d]=%d\n", num, gv_refrence_node[num]);
			if(num == 0)
			{
				min_value = max_value = gv_refrence_node[num];
			}
			else
			{
				if(num < NODE_TOTAL_NUM - NODE_Y_NUM)
				{
					if (gv_refrence_node[num] > max_value)
					{
						max_value = gv_refrence_node[num];
					}
					if (gv_refrence_node[num] < min_value)
					{
						min_value = gv_refrence_node[num];
					}
				}
			}
		...
		diagnostic_chip(info, QT_PAGE_UP);
		msleep(20);
	}

#if 0
	printk("[TSP] Reference Value : ");
	for(i = 0 ; i < 19*11 ; i++)
	{
		printk("%d ", gv_refrence_node[i]);
	}
	printk("\n");
#endif

//	printk("min_value=%d, max_value=%d ",min_value, max_value);
...
static void run_delta_read(void *device_data) //item2
{
	...
	diagnostic_chip(info, QT_DELTA_MODE);
	msleep(30);		/* msleep(20);  */
	...
	for (read_page = 0; read_page < 4; read_page++) {
		for (node = 0; node < 64; node++) {
			read_point = (node * 2) + 2;
			read_mem(info, object_address + (u16) read_point, 2, data_buffer);
			info->qt_delta_node[num] = ((uint16_t) data_buffer[1] << 8) + (uint16_t) data_buffer[0];

			gv_delta_node[num] = (s16) info->qt_delta_node[num];
...
#if 0
	printk("[TSP] Delta Value : ");
	for(i = 0 ; i < 19*11 ; i++)
	{
		printk("%d ", gv_delta_node[i]);
	}
	printk("\n");
#endif
...
{
	struct mxt224_data *info = dev_get_drvdata(dev);
				  ...
				  const struct i2c_device_id *id)
{
	...
	for (i = 0; i < 2; i++)
	{
		ret = read_mem(data, 0, sizeof(ids), ids);

		if (ret) // fail
		{
			printk("[TSP] %s, i2c communication failed\n", __func__ );
		}
		else  //sucess
		{
			printk("[TSP] %s, i2c communication success\n", __func__ );
			break;
		}
	}
...
//	data->pdata->power_on();
	/* Add delay to prevent problem blocking transition to idle screen because of chip initialization failed in mxt224 */
...
*/
	...
	/* config tunning date */
	data->tsp_config_version = "S2VEP_At_0724";
	...
	if (ret) {
		pr_err("[TSP] reset command failed. ret=%d", ret);
		/*goto err_reset */
	}
	...
	data->noise_median.mferr_setting = false;
	data->noise_median.mferr_count = 0;
	data->noise_median.t46_actvsyncsperx_for_mferr = 40;//38->40 20120621
	...
	data->noise_median.t48_movfilter_for_mferr = 81; //80->81 20120621
#endif
	...
	ret = request_threaded_irq(client->irq, NULL, mxt224_irq_thread,
			IRQF_TRIGGER_FALLING, "mxt224_ts",
			data);
...
*/
	...
	if (device_create_file(sec_touchscreen, &dev_attr_tsp_firm_version_config) < 0)
		pr_err("Failed to create device file(%s)!\n",
